...
<html lang="en" data-bs-theme="auto">
	...
	<body class="bg-body-tertiary">
			...
			<div id="agreementRecheck"
				...
				<form>
					<div class="form-check">
						<input type="checkbox"
							class="form-check-input border border-primary" id="checkbox" name="checkbox">
		...
		<script>
		 ...
		 <!-- 체크박스를 활성화할 조건을 확인하는 스크립트 -->
      <script>
        var checkbox = document.getElementById('checkbox');
        var withdrawalTxt = document.getElementById('withdrawal1-txt');

        // 텍스트 에어리어에 텍스트가 채워질 때 스크롤 다운
        withdrawalTxt.addEventListener('input', function () {
          // 스크롤 다운
          withdrawalTxt.scrollTop = withdrawalTxt.scrollHeight;

          // 약관 내용이 다 내려왔을 때 체크박스 활성화
          if (withdrawalTxt.scrollTop === withdrawalTxt.scrollHeight - withdrawalTxt.clientHeight) {
            checkbox.disabled = false;
          } else {
            checkbox.disabled = true;
            checkbox.checked = false; // 스크롤 올라갔을 때 체크 해제
          }
        });

        // 체크박스 클릭 시 스크롤 상태 확인 후 처리
        checkbox.addEventListener('click', function () {
          if (withdrawalTxt.scrollTop < withdrawalTxt.scrollHeight - withdrawalTxt.clientHeight) {
            alert('약관을 모두 읽고 체크해 주세요.');
      ...
      <script>
		  function confirmWithdrawal() {
		    ...
		    if (!checkbox.checked) {
		      ...
		      // 체크박스가 체크되어 있고, 사용자가 확인 창에서 Yes를 선택한 경우
